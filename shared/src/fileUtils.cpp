...
bool fileExists(const Path& path, const bool verbose) {
	return std::filesystem::exists(path)
		? verbose ? log(L"File found at " + path.wstring()) : true 
		: verbose ? log(L"File not found at " + path.wstring(), true) : false;
}
bool deleteFile(const Path& path) {
	if (fileExists(path, true) && !std::filesystem::remove(path)) 
...
bool deleteDirectory(const Path& path) {
	if (fileExists(path, true) && !std::filesystem::remove_all(path)) 
